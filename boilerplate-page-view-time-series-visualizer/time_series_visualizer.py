...
def draw_bar_plot():
    ...
    df_bar_new = df_bar.groupby([df.index.year, df.index.month],)['value'].agg(np.mean).rename_axis(['year', 'month'])
    df_bar_new = df_bar_new.reset_index()

    df_pivtab = pd.pivot_table(df_bar_new,
    values = 'value',
    index = 'year',
    columns = 'month')
